...
class TAGCN(MapReduceGNN):
    ...
    def build(self, input_shapes):
        ...
        self.kernel = self.add_weight("kernel", shape=[num_features * (self.K + 1), self.units], initializer="glorot_uniform")
        if self.use_bias:
    ...
    def call(self, inputs, cache=None, training=None, mask=None):
        ...
        for k in range(self.K):
            out = tagcn(xs[-1], edge_index, edge_weight, renorm=self.renorm, improved=self.improved, cache=cache)
            xs.append(out)
