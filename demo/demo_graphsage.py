...
import numpy as np
from tf_geometric.nn.conv.graphsage import Mean_Aggregator, GCN_Aggregator, MeanPooling_Aggregator, MaxPooling_Aggregator
...
aggregator_type = MaxPooling_Aggregator
print('aggregator_type:',aggregator_type)

graphsage = GraphSAGE(hidden_units=128, aggregate_fun=aggregator_type)
...
def forward(graph, num_layers, sizes=None, training=False):
    ...
    h = graphsage([graph.x, edge_index_list[0], edge_weight_list[0]], cache=graph.cache)

    h = dropout(h, training=training)

    for i in range(1,num_layers - 1):
        h = graphsage([h, edge_index_list[i], edge_weight_list[i]])

        if i != num_layers - 1:
            h = dropout(h, training=training)
    h = graphsage([graph.x, edge_index_list[-1], edge_weight_list[-1]])
